...
import data.Roles
import data.User
import services.ArgumentsParser
...
class App {
    ...
    fun run(args: Array<String>): Int {
        val parser = ArgumentsParser("handler")
    ...
    private fun authentication(login: String, pass: String): Pair<Int, User?> {
        if (!isLoginValid(login))
            return INVALID_LOGIN_FORM.exitCode to null

        val user = dbWrapper.getUser(login)
        return when {
            user == null -> UNKNOWN_LOGIN.exitCode to null
            !isPasswordValid(pass, user.salt!!, user.hashPassword!!) -> INVALID_PASSWORD.exitCode to null
